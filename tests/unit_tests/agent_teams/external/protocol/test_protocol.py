...
class _Harness:
    ...
    async def turn_events(self) -> AsyncIterator[HarnessEvent]:
        turn_id = None
        async for event in self.events():
            payload = event.event
            if turn_id is None:
                if not (isinstance(payload, TurnLifecycleEvent) and payload.kind is TurnEventKind.STARTED):
                    continue
    ...
    async def send(
        ...
        *,
        ...
        _ = content
        return SendReceipt(message_id="message-1", accepted_mode=mode)
...
def test_card_is_immutable_and_reports_capabilities() -> None:
    ...
    assert card.protocol_version == PROTOCOL_VERSION == "3.0"
...
async def test_checkpoint_sink_accepts_proactive_updates() -> None:
    ...
    await sink.save(checkpoint, reason=CheckpointReason.TURN_COMPLETED)
...
def test_checkpoint_requires_provider_version_and_member_scope() -> None:
    with pytest.raises(ValueError, match="provider must not be empty"):
        HarnessCheckpoint(provider="", schema_version="1", member_agent_id="member-a")
    with pytest.raises(ValueError, match="schema_version must not be empty"):
        HarnessCheckpoint(provider="acme", schema_version="", member_agent_id="member-a")
    with pytest.raises(ValueError, match="member_agent_id must not be empty"):
        HarnessCheckpoint(provider="acme", schema_version="1", member_agent_id="")
...
async def test_turn_events_is_finite_and_includes_terminal_event() -> None:
    harness = _Harness()
    response = [event async for event in harness.turn_events()]
    remaining = [event async for event in harness.events()]

    assert len(response) == 3
    assert [event.sequence for event in response] == [2, 3, 4]
